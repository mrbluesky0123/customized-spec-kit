...
3. 다음 실행 흐름을 따르십시오:
    ...
    4. **필수 기술 표준 확인**: 사용자가 명시하지 않은 경우 다음 항목을 **반드시 사용자에게 질문**:

        **중요: 이 항목들은 추측하거나 임의로 결정하지 말고 반드시 사용자에게 확인해야 함**

        질문 예시:
        - 자바 서버 프로젝트인 경우: "맛간장(자바 서버 표준) 사용 여부를 알려주세요. 사용하지 않는다면 그 이유도 함께 설명해주세요."
        - "감사 로그 적재가 필요한가요? 필요하다면 LMS(카카오뱅크 감사로그 적재 시스템)를 사용하시나요?"
        - Admin 화면을 포함하는 프로젝트인 경우: "EAM(Enterprise Access Management) 적용이 필요한가요?"

        확인된 내용(사용자 응답 및 적용/미적용 사유)은 반드시 `specs/project-spec-init/clarifications.md`에 문서화
    5. **시스템 아키텍처 확인**: 시스템 구성도 및 컴포넌트 설명을 작성하기 전에 **반드시** 사용자에게 다음을 질문:

        **중요: 시스템 아키텍처는 추측하지 말고 반드시 사용자에게 확인해야 함**
        ...
        사용자 응답을 받은 후에만 이를 기반으로 시스템 구성도(ASCII 다이어그램) 및 컴포넌트 상세 설명 작성
    ...
    6. 시스템 요구사항 생성
        ...
        **시스템 요구사항에 포함하지 말아야 할 것**:
        ...
        - **비즈니스 정책 - WHEN/WHAT 중심 설명** (아래 잘못된 예시 참조)
        ...
        - "SR-006: 재고 차감은 JPA Optimistic Locking을 통해 동시성을 제어해야 함"
        - "SR-007: 주문 취소 시 재고 복구는 @Transactional 내에서 원자적으로 처리되어야 함"

        ❌ 잘못된 예시 - 비즈니스 정책 (WHEN/WHAT 중심):

        - "시스템은 상품 조회 API를 제공해야 함" → 비즈니스 명세 (WHAT)
        - "주문 API는 POST /api/v1/orders 엔드포인트로 구현되어야 함" → Plan 단계
        - **"재고 차감은 주문 생성 시점에 수행되어야 하며..." → 비즈니스 명세 (WHEN)**
        - **"주문 취소 시 재고는 자동으로 복구되어야 하며..." → 비즈니스 명세 (WHAT)**
        - **"결제 실패 시 주문은 자동으로 취소되어야 함" → 비즈니스 명세 (WHAT)**

        **시스템 요구사항 자가 검증 체크리스트**:

        각 시스템 요구사항을 작성한 후 아래 질문에 답하십시오. **하나라도 YES면 해당 요구사항은 비즈니스 명세로 이동**:

        1. 이 요구사항이 "언제(WHEN)" 어떤 일이 일어나는지 설명하는가?
           - 예: "주문 생성 시점에", "결제 실패 시", "취소 시"
        2. 이 요구사항이 "무엇을(WHAT)" 제공하는지 설명하는가?
           - 예: "API를 제공해야 함", "자동으로 복구되어야 함"
        3. 이 요구사항에서 구체적인 기술/프레임워크/패턴이 명시되어 있지 않은가?
           - 예: JPA, @Transactional, Locking, Mock, REST 등의 기술 용어 없음
        4. 이 요구사항이 사용자 관점에서 이해 가능한가? (개발자 아닌)
           - 예: "재고가 복구되어야 함"은 사용자도 이해 가능 → 비즈니스 정책

        **올바른 시스템 요구사항은 "HOW(어떻게)" 중심**:
        - "JPA Optimistic Locking을 사용하여"
        - "@Transactional 내에서 원자적으로"
        - "Mock 서비스로 구현하며, 1초 지연"
